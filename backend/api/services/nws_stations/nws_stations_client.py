...
class NWSStationsConfig(BaseModel):
    ...
    user_agent: str = os.getenv(
        "NWS_USER_AGENT",
        "EVAonline (+https://github.com/evaonlineplatform/EVAONLINE)",
    )
    # max_stations: int = 10
...
class NWSStationsClient:
    ...
    async def get_observations(
        ...
        for attempt in range(self.config.retry_attempts):
            try:
                ...
                logger.warning(
                    f"NWS Stations request failed "
                    f"(attempt {attempt + 1}): {e}"
                )
                if attempt == self.config.retry_attempts - 1:
                    logger.error(
                        f"Failed to get observations from {station_id}: {e}"
                    )
                    return []
    ...
    async def _delay_retry(self, attempt: int = 0):
